...
通常情况下，各云服务商针对自身服务器会提供有部分监控功能，然而其大多仅局限于部分硬件指标，难以进行灵活扩展，企业很难结合自身应用进行监控项的定制；除此之外，各云服务商的监控服务也不尽相同，直接混用会使得企业内部的通知告警杂乱无章，影响异常发生时服务器故障的定位与排查。除了云服务商提供的监控服务以外，虽然Linux系统本身也提供有各类性能监控工具，但是其多用于实时查看系统状态，如果想要更进一步对服务器的历史数据进行分析，或是对服务器集群进行批量监控，设置告警阈值，这些性能监控工具往往无法满足运维管理的需要。  
...
目前，国内外多数公司都投入了大量的研究在服务器集群的监控上以实现对公司产品的支持，维护产品的可靠性和提高系统性能。其中用于商业的常见云平台监控系统有Monitis、CloudStatus、CloudWatch等，他们各自的侧重点存在一定的差异。如用于监测亚马逊EC2和S3云存储的Monitis，能用采用代理的方式，把监控自动地安装到服务器上开展对服务器资源的监控工作，并在发现问题时自动报警。此外，它还能对性能、系统等进行监控，提供配置管理功能。Monitis性价比较高，对系统及绩效的检测有着良好的效果；亚马逊针对自身的AWS云服务，开发了CloudWatch来提供监控服务，免费为用户提供自动缩放的功能及动态迁移或增加EC2实例。CloudWatch以AWS云资源的日志文件及各种指标为监控和收集的目标，监控的云资源包括Amazon的EC2实例、RDS数据库实例、DynamoDB表、日志文件等；阿里巴巴集团经过多年的监控研究，结合阿里云云计算平台强大的数据分析能力开发了阿里云-云监控。它通过第三方形式，提供应用及服务器的监控服务给一些中小型的网站，保障用户产品及业务的顺利运行。
除了商业产品之外，开源社区也对监控系统有着大量的研究，一些开源工具正不断地吸引越来越多人的眼球，同时也被很多公司应用到实际的系统监控中，包括Cacti、Zabbix、Ganglia、Hyperic-HQ、CloudStack、Nagios等。Cacti是基于PHP、SNMP及RRDTool的网络流量监测分析工具，具有强大的用户权限和数据管理功能，可以指定每一个用户能查看的树状结构及任何一张图。Ganglia主要用于服务器系统性能的监控如硬盘的使用、负载等，能通过管理页面的监控曲线比较清楚的知道监控系统中节点的运行状态，从而实现对云平台中的资源合理分配和调整。Ganglia分层的架构模型使其具有良好的扩展性，支持高并发、跨平台且性能开销低，能较好的满足服务器监控的需要；Nagios最开始于1999年以项目名NetSaint发布出来，其最大的优势在于其完善的插件机制，企业可以根据公司的实际需求来定制监控功能,它正被众多大公司应用在生产环境中，如西门子，PHILIPS，雅虎等。
